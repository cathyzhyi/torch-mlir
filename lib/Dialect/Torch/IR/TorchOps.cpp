...
using ConstantIntComparator = std::function<bool(int, int)>;
template <typename OpTy>
static OpFoldResult comparatorFoldHelper(OpTy op, bool equalIsTrue,
                                         ConstantIntComparator comparator) {
  if (op.getOperand(0) == op.getOperand(1))
    return getI1IntegerAttr(op.getContext(), equalIsTrue);
...
OpFoldResult AtenNeIntOp::fold(ArrayRef<Attribute> operands) {
  return comparatorFoldHelper(*this, /*equalIsTrue=*/false,
...
OpFoldResult AtenEqIntOp::fold(ArrayRef<Attribute> operands) {
  return comparatorFoldHelper(*this, /*equalIsTrue=*/true,
                              [](int a, int b) { return a == b; });
}
