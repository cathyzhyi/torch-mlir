...
private:
  LogicalResult collectUsedSlots() {
    ...
    symbolTable.getOp()->walk([&](PrimGetAttrOp op) {
      auto moduleType = op.receiver().getType().cast<NnModuleType>();
      ...
      auto slotIt = nameToSlot.find(op.name());
      if (slotIt == nameToSlot.end())
        op.emitError() << "Reference to non-existing module slot " << op.name()
